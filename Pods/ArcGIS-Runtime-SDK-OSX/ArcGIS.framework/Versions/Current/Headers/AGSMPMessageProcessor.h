...
/**  A message processor object.
 
 A message processor allows you to process a  AGSMPMessage from external resources.
 For example, display a military symbol by passing a 2525C message to a
 message processor. 
 
 Each message processor comes with a  AGSMPSymbolDictionaryType
 ...
 A message processor is bound to a
  AGSGroupLayer, which is used to display, update and remove  AGSMultiLayerSymbol
 associated with the message. The group layer has to be added into  AGSMapView, otherwise
...
/** Creates a new  AGSMPMessage from an  AGSGraphic object. The message gets populated with
 ...
 @return An autoreleased  AGSMPMessage populated with attributes/geometry from the specified graphic.
 @since 10.2
 */
- (AGSMPMessage*)messageFromGraphic:(AGSGraphic*)graphic;

/** Creates a new  AGSMPMessage from an existing message id. The message id must have
 ...
 @return An autoreleased  AGSMPMessage populated with attributes/geometry from the previous message.
 @since 10.2
 */
...
/** Retrieves the  AGSGraphic which originates from the message with specified id.
