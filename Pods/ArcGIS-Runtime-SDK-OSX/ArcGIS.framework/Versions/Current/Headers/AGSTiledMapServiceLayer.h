...
/**  A layer based on a tiled ArcGIS Map service.
 ...
 An instance of this class allows you to display maps from a tiled ArcGIS Map service. The service may be hosted in the cloud on ArcGIS Online or on-premise with ArcGIS Server. For example http://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer
 A tiled map service uses a cache of pre-generated tiles to create a map instead of
...
/** URL of a tiled ArcGIS map service
 For example, http://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer
 @since 10.2
 */
...
/** Initialize this layer with a URL to an ArcGIS Server cached map service with 
 ...
 @param cred  AGSCredential used to access the secured resource.
 @return A new tiled map service layer object.
 @since 10.2
 @see  [AGSLayerDelegate layerDidLoad:] , method on delegate for success
 @see  [AGSLayerDelegate layer:didFailToLoadWithError:] , method on delegate for failure
...
/** Initialize this layer with an  AGSMapServiceInfo object.
 You can use this method if you already have an  AGSMapServiceInfo so 
...
/** A method to get an initialized, autoreleased map service layer using 
 an  AGSMapServiceInfo object. Use this method if you already have an 
  AGSMapServiceInfo so that another round trip to the server does not occur to 
