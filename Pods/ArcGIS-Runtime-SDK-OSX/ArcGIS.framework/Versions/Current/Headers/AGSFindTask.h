...
/**  A task to perform simple, textual searches 
...
/**  A delegate of  AGSFindTask.
 
 A protocol which must be adopted by any class wishing to be notified when the 
  AGSFindTask completes successfully or encounters an error. An instance of 
 the class must then be set  [AGSFindTask delegate] .
...
@required
/** Tells the delegate that  AGSFindTask completed successfully with the 
 provided results.
 @param findTask The task which performed the search.
 @param op <code>NSOperation</code> which executed the find task.
 @param results The resulting array of  AGSFindResult objects.
 */
