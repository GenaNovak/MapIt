...
/**  A task to perform identify operations.
 ...
 @see oncept{identify-task.htm, Using an Identify Task}
 @see 
 */
...
/** Performs an identify operation on the map service using the specified 
 ...
 @since 10.2
 @see  [AGSIdentifyTaskDelegate identifyTask:operation:didExecuteWithIdentifyResults:,] method on delegate for success
 @see  [AGSIdentifyTaskDelegate identifyTask:operation:didFailWithError:] , method on delegate for failure
...
/**  A delegate of  AGSIdentifyTask.
 
 A protocol which must be adopted by any class wishing to be notified when the 
  AGSIdentifyTask completes successfully or encounters an error. An instance 
 of the class must then be set as  [AGSIdentifyTask delegate] .
...
@required
/** Tells the delegate that  AGSIdentifyTask completed successfully with the 
 provided results.
 @param identifyTask The task which performed the operation.
 @param op <code>NSOperation</code> that performed the identify.
 @param results The resulting array of  AGSIdentifyResult objects.
 @since 10.2
 */
