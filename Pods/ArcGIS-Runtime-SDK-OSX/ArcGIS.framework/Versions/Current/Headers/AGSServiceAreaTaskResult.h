...
/** The facilities (points) as an array of  AGSGraphic objects. These are only returned if the <code>returnFacilities</code> property
 on  AGSServiceAreaTaskParameters is set to <code>true</code>.
 @since 10.2
 */
@property (nonatomic, copy, readonly) NSArray *facilities;

/** Messages as an array of  AGSNAMessage objects. These messages are received when solve is completed. If a service area cannot be solved, 
...
/** The point barriers as an array of  AGSGraphic objects. They are returned only if 
 ...
 default). If you send in the point barriers as a featureSet (instead of using 
  AGSDataLayer), you already have the barriers and might not need to request 
 them back from the server.
 @since 10.2
 */
@property (nonatomic, copy, readonly) NSArray *pointBarriers;

/** The polygon barriers as an array of  AGSGraphic objects. They are returned only if 
 ...
 using  AGSDataLayer), you already have the barriers and might not need to 
 request them back from the server.
 @since 10.2
 */
@property (nonatomic, copy, readonly) NSArray *polygonBarriers;

/** The polyline barriers as an array of  AGSGraphic objects. They are returned only if 
